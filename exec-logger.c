...
int execve(const char *progname, char *const argv[], char *const envp[]) {
  ...
  fp = fopen(loginuid, "r");
  if (fp == NULL) {
    syslog(LOG_WARNING, "ERROR: Couldn't open /prox/%d/loginuid", getppid());
  } else {
    if (fscanf(fp, "%d", &uid) != 1) {
      syslog(LOG_WARNING, "ERROR: Couldn't read /proc/%d/loginuid", getppid());
      uid = -1;
    }
  ...
  sprintf(buf, "[UID:%d PARENT_PID:%d] CMD:", uid, getppid());
